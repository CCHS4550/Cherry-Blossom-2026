...
public class Drive extends SubsystemBase {
  ...
  //  * Pid Controller for drive at angle.
  //  *
  //  * <p>creates a new pid controller with built in trapezoidal motion this is necesary because
  // while
  //  * the pid controller built into the turn motor can handle turning a singular wheel to an
  // angle, a
  //  * seperate pid must be called to give the overall angles that the bot is hitting
  //  */
  // ProfiledPIDController angleController =
  ...
  //     false; // exists in order to prevent the periodic state machine from calling the same
  // command
  // // multiple times
  // private BooleanSupplier shouldCancelEarly =
  //     () ->
  //         false; // we can enable should cancel early anytime we want to stop a command from
  // running
  ...
  // // creates a swervedrive pose estimator, can be used to fuse with vision and does the math
  // needed
  // // to update given our bots information
  // private SwerveDrivePoseEstimator poseEstimator =
  //     new SwerveDrivePoseEstimator(kinematics, rawGyroRotation, lastModulePos, new Pose2d());
  ...
  //     for (int moduleIndex = 0; moduleIndex < 4; moduleIndex++) {
  //       modulePositions[moduleIndex] =
  //           modules[moduleIndex].getOdometryPositions()[i]; // set the position at given
  // timestamp
  ...
  //       runSysID();
  //       break;
  //     case AUTO: // AUTO just automatically breaks because it is made of a completely planned set
  // of
  //       // commands to follow that don't require a periodic state system
  //       break;
  //     case TELEOP_DRIVE:
  ...
  // /**
  //  * turns a set of overall robot relative speeds into individual module instructions for the
  // motors
  //  * to then follow
  //  *
  //  * @param speeds the desired chassis speeds, in XY units of meters per second and omega units
  // of
  //  *     radians per second
  //  */
  ...
  //       setPointStates,
  //       Constants.DriveConstants
  //           .maxSpeedMetersPerSec); // normalizes wheel velocity if any individual modules are
  // above
  //   // the max speed. Keep in mind that if this is called, the
  //   // discretization will be innaccurate
  ...
  //   // set all modules to our found states. Note that we still have to optomize our wheel angle
  // for
  //   // better wraparound
  //   // set to 3 to ignore the broken swerve module? fix once fixed
  //   for (int i = 0; i < 4; i++) {
  ...
  // /**
  //  * turns a set of overall speeds into individual module instructions for the motors to then
  // follow
  //  * but with a max turning velo
  //  *
  //  * @param speeds the desired chassis speeds, in XY units of meters per second and omega units
  ...
  //   // limits our requested rotational speed to a maxium velocity before desscretizing to avoid
  // any
  //   // unintentionalskew
  //   // simply setting a max cap and not a scale because I dont care how it gets up to this max
  // velo
  //   // keep in mind that this is potential bad practice or a misinterpretation of the following
  //   // methods
  //   if (speeds.omegaRadiansPerSecond > maxTurnVelocityRadiansPerSecond) {
  ...
  // }
  // /**
  //  * This method takes in our various joystick inputs, and converts them into a chassis speed
  // while
  //  * applying deadband then, the bot is set to run at the speeds everything should be field
  // relative
  //  *
  //  * @param xInput the horizontal joystick input
  //  * @param yInput the vertical joystick input
  ...
  // }
  // /**
  //  * same thing as the other getLinearVeloFromXY method but applies deadband for teleop converts
  // a
  //  * set of x and y speeds into a singular linear velocity - should be field oriented
  //  *
  //  * @param x the horizontal speed, units are arbitrary but should be meters per second for
  ...
  //   for (int i = 0; i < 4; i++) {
  //     headings[i] =
  //         Constants.DriveConstants.moduleTranslations[i].getAngle(); // sets the module angles to
  // x
  //   }
  //   kinematics.resetHeadings(headings); // applies the headings
  //   stop();
  ...
  // }
  // /**
  //  * In this test, a constant ‘step voltage’ is given to the mechanism, so that the behavior
  // while
  //  * accelerating can be determined.
  //  *
  //  * @param direction whether to perform this forward or reverse
  ...
  // /**
  //  * getter for our current swerve module position very similar to module states, but state
  // measures
  //  * drive velocity, while positions measures distance moved
  //  *
  //  * @return an array of all 4 swerve module positon
  ...
  // /**
  //  * log and getter for the position of our drive wheel when performing our wheel
  // characterization
  //  * said test is not yet written
  //  *
  //  * @return an array of drive motor information for characterizing wheel radius
  ...
  // /**
  //  * sets the bot's wanted state should be the primary way of manipulating the drivetrain outside
  // of
  //  * the class
  //  *
  //  * @param wantedState the desired state
